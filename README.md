...
- **Icons & product logos** — pick from a built-in [Tabler](https://tabler.io/icons) icon set (self-hosted, works offline), search thousands of real **app/product logos** (Plex, Sonarr, Proxmox, …), or paste any image URL.
- **Customisable tiles** — give each tile its own width, height, colour (a preset or a custom colour), icon size, and name/subtitle font size.
- **Multi-user with roles** — `admin` (can edit everything) and `viewer` (view only); each user has their own profile avatar.
